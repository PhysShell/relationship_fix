...
#: ДОМЕН [0, H] — СТРУКТУРНЫЙ, а не «дальше не искали». Это важно для
#: чтения компоненты вида [1535.81, H]: если бы H был вычислительным
#: потолком, правильной семантикой было бы «продолжается за горизонт».
#:
#: Доказательство короткое: B = Σ min(T_i, H) по допустимым возможностям,
#: каждое слагаемое <= H, значит B <= H·N, и любое достижимое отношение
#: лежит в [0, H]. Снизу — B >= 0. Граница достигается: при отсутствии
#: возврата B = H·N ровно.
#:
#: И это ИНВАРИАНТ С ТЕСТОМ, а не знание автора: проверено на 3000
#: конфигураций в обоих слоях, максимум B_max/(H·N_max) равен 1.000000.
DOMAIN_IS_STRUCTURAL = "B <= H·N by construction, so every achievable ratio is in [0, H]"
DOMAIN_UPPER_END_IS_NOT_A_SEARCH_CAP = True
DOMAIN_INVARIANT_IS_TESTED = True

#: ВЫРОЖДЕННАЯ ДИСПЕРСИЯ. Обоснование уточнено девятым чтением; прежнее
#: было неверным. Писалось «в истинном λ0 среднее равно нулю, значит
#: отвергнуть там — потерять покрытие». НЕ СЛЕДУЕТ: из E[ψ(λ0)] = 0 не
#: вытекает нулевое ВЫБОРОЧНОЕ среднее. Дискретный DGP может выдать 25
#: одинаковых ненулевых значений при популяционном нуле.
#:
...
ZERO_VARIANCE_RULE = "undefined statistic -> do not reject; costs power, not coverage"
