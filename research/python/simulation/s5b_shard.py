...
def groups_needed(rate: float, look: int) -> int:
    ...
    Числа здесь — ПОСЛЕ калибровки по настоящему раннеру. Самая дорогая
    рука сетки (`r96.0:c1.25:R0:m1.0`, эффективная ставка 120) на третьей
    ступени одной группой требует 8.48 ч, то есть выше платформенного
    потолка 6 ч; трёх групп хватает — 3.11 ч.

    До калибровки те же величины читались как 6.87 ч и две группы. Оценка
    по машине разработчика давала 5.5 ч, то есть «влезает», и именно этот
    ответ проба и опровергла.
    """
