...
FINDINGS: tuple[tuple[str, CheckVerdict, str], ...] = (
    ...
    ("events.duplicate_semantics", CheckVerdict.PASSED,
     "Оба участника пользовались выданными телефонами, и сообщение между ними "
     "могло попасть в оба лога. Проверено эмпирически: в гистограмме пауз между "
     "подряд идущими сообщениями ОДНОГО направления нет всплеска на 0-1 с "
     "(21, 46, 107, 137, 153 — плавный подъём, а не пик). Двойного логирования "
     "нет, одна запись на сообщение."),
    ("identity.resolution_quality", CheckVerdict.PASSED,
     "Идентичность НЕ вероятностная: целочисленные идентификаторы участников "
     "назначены исследованием, полей уверенности нет, выбирать порог не "
     "приходится."),
    ("channel.mixing", CheckVerdict.PASSED,
     "Один канал: sms.csv содержит только SMS. Звонки лежат отдельным файлом и "
     "не подмешиваются. Платформа у всех одна — выданный Nexus 4."),
