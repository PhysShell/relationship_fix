...
FINDINGS: tuple[tuple[str, CheckVerdict, str], ...] = (
    ...
    ("scale.counts", CheckVerdict.PASSED,
     ...
     "сглажено."),
    ("coverage.semantics", CheckVerdict.FAILED,
     "ЧАСТИЧНО. Установлено: «non-participants were removed», значит пустой "
     "промежуток НЕ означает молчание — переписка с людьми вне исследования "
     "вырезана, и наблюдаемая incidence есть строгая нижняя граница настоящей. "
     "НЕ установлено: окно участия каждого пользователя (телефон выключен, "
     "выход из исследования, поздний вход). Без него incidence считается по "
     "номинальным 28 суткам для всех, и отсутствие данных становится тишиной — "
