...
#: ИТОГ ПРОБЫ (полный файл, 60 486 564 строки, 587 участников):
#:
#:     D0 точных зеркал (та же миллисекунда) = 1 362 951
#:     D1 близких зеркал (|dt| <= 5 с)       = 1 992 455
#:     D2 ожидание по случайности            =     6 470
#:     превышение                            =       308x
#:
#: Гистограмма |dt|: 0 с — 1 421 754, дальше 210 237 / 94 304 / 94 350 / 93 002
#: / 78 808. Пик у нуля, как и предсказывала сигнатура.
#:
...
#:
#: Мина была настоящая. Без сведения пары участник-участник выглядели бы вдвое
#: плотнее, чем пары участник-внешний, где вторая копия невозможна в принципе.
#: Это СМЕЩЕНИЕ РАЗНОСТНОЕ, а не равномерное, и потому хуже: оно подделало бы
#: не масштаб, а саму зависимость плотности от типа собеседника.

#: Правило сведения, объявленное вместе с находкой.
#:
#: Документация говорит, как переиндексировать: «they need to use the outgoing
#: variable to reindex the records». Дальше выбор наш, и он такой: копию с
#: устройства ОТПРАВИТЕЛЯ (`outgoing=Yes`) оставляем, зеркальную копию с
#: устройства получателя (`outgoing=No`) в пределах окна отбрасываем.
#:
#: Почему отправитель: его аппарат — непосредственный свидетель момента
#: отправки, а запись на стороне получателя отстоит на доставку и на
#: расхождение часов. Хвост гистограммы (210 тысяч на 1 секунде и дальше) —
#: это и есть та задержка, и брать её за момент события значило бы
#: систематически сдвигать латентность вперёд.
DEDUP_KEEP = "sender_side"
