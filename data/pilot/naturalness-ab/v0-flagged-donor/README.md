...
Итог: 11 accepted, 2 vetoed → переписаны: **pc-12-d3** («ok but next time text me when plans change. every time. I mean it» — возвращено исходное требуемое поведение, без temporal contract) и **pc-17-d2** («Ты отменил встречу и даже не позвонил. Ну нормально вообще?» — обида без emotion-report и без нового факта; «встречу» сохранено, чтобы не терять факт контекста). Оба **ждут verdict**. Запись каждого решения — `facilitator_review` у кандидата (`outcome: accepted | vetoed | not_reviewed`, причина, `at_build`), сводка — `review_log[1]`.

Спорные места, названные в самих новых кандидатах: pc-12-d3 лексически близок к оригиналу (донорский вклад — только форма «ok + механизм на следующий раз + хвост настойчивости»), унаследованная латентная неувязка «B в этот раз написал» сохранена сознательно по verdict'у; pc-17-d2 — риторический вопрос чуть ослабляет adjacency неизменного «Понимаю.» (V7 мягкое замечание) и сдвигает обиду к возмущению; запасной вариант без вопроса — повтор «Даже не позвонил.» (в checker note).

## Что не сделано

- 11 кандидатов приняты фасилитатором с записанной причиной (см. Verdicts); pc-12-d3 и pc-17-d2 ждут verdict. Принятие записано только здесь: **корпус не изменён, v0.1 не собран** — сборка v0.1 начинается после 13/13 verdicts и отдельного «го».
