...
- **Fallback при нехватке людей (2026-09-08):** единственный разрешённый — сначала эта generation 1 на трёх оценщиках, затем один из них аудирует только 33 items вне A/B; записывается в `contamination-ledger.json` до выдачи; потеря evidence описана в README аудита. Оценщик заражён для своих 13 items, не глобально.
- **Поверхность:** XLSX — текущая operational human surface, не временная; веб придёт с cutover v0.1 как тупой renderer/collector, привязанный к sha256 пакета и токену, без собственной рандомизации.
- **Контакт для обратной связи:** отложен до момента, когда ссылки или файлы пойдут через третьих лиц. Тогда — необязательное поле с отдельным consent, вне research export, удаляемое отдельно, связанное с ответами только через псевдоним у фасилитатора и никогда не используемое как quasi-identifier в анализе. Пока выдача личная, канал уже есть.
