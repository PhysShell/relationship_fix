...
5. Issuance records: `metrics.issuance new` (после заполненного `eligibility.json`) пишет `issuance/annotator-N.json` с sha256 токена и всех выданных байтов и печатает токен один раз; каталог `issuance/` здесь пока не существует — записи появляются только при J. Экспорт слоя: `annotation-web-export annotation-pilot-v0.1 annotator-N responses/` → `responses/annotator-N.jsonl` + `annotator-N.export.json`.
