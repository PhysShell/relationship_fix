...
4. Web cutover — сделан 2026-09-08 (отдельная приёмка, см. [cutover contract](../../../docs/pilot-v0.1-cutover-contract.md)): реестр статусов [`data/pilot/package-registry.json`](../package-registry.json) (v0 `frozen_non_issuable`, этот пакет `issuable` с pin на `CHECKSUMS.sha256`), token-bound renderer/collector в annotation-web, который читает из этого каталога **только** `CHECKSUMS.sha256` и `presentation/annotator-N.jsonl`, pc-08 в glossary `Domain.hs` перефразирован, `Catalog.hs` на dogfood v7.
5. Issuance records: `metrics.issuance new` пишет запись с sha256 токена и всех выданных байтов и печатает персональную ссылку один раз; каталог `issuance/` здесь не существует — записи это host state (`/var/lib/relationship-fix/issuance`). **Eligibility не в этом пакете:** `eligibility.json` здесь — null-шаблон, запечатанный вместе с пакетом, и он не редактируется никогда (правка ломает seal, а сломанный seal = отказ в issuance). Фактическая eligibility человека — внешняя запись `rf.annotator-eligibility.v1` по приватному пути; `new` отказывает, пока каждый критерий из manifest'а там не равен ровно true, и пишет sha256 этой записи в issuance record. Порядок шагов — runbook, раздел «Issuing a person (J)». Экспорт слоя: `annotation-web-export annotation-pilot-v0.1 annotator-N responses/` → `responses/annotator-N.jsonl` + `annotator-N.export.json`.
