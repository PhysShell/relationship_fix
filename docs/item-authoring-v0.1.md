...
6. **Natural-страта не обогащается и не правится под label.** Она существует ради base rates; любая правка в ней — только лексическая и только через acceptance-путь пакета: по результату A/B, а на пути без людей — facilitator acceptance с записанной причиной (решения 2026-09-08; прецедент pn-10-d1).
