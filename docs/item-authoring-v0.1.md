...
6. **Natural-страта не обогащается и не правится под label.** Она существует ради base rates; любая правка в ней — только лексическая и только по результату A/B.
...
8. **Naturalness-triage до freeze.** Перед заморозкой пакета каждый item проходит rubric критика (gate §5). Flagged items либо переписываются автором до freeze (это `human`-origin с `revision_reason`), либо идут в blinded A/B. После freeze — только новая версия пакета.
