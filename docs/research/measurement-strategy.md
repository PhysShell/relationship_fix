...
## 10. Дополнение 2026-09-17 — чем закончилась синтетическая ветка
...
Цепочка вопросов, по которой проект сюда пришёл, — она же объяснение, почему люди понадобились только сейчас:

```
как лучше анализировать переписку?
    → что в переписке вообще наблюдаемо?
        → что из наблюдаемого люди воспроизводимо видят?
            → совпадает ли наблюдаемое с опытом получателя?        (TRACK 4: нет)
                → можно ли получить этот опыт, не изменив его самим измерением?
```

На последний вопрос не отвечает ни один существующий датасет — по построению. Поэтому первый эксперимент с людьми не про нашу онтологию, а про то, не портит ли измерение измеряемое.
