...
| 8 | Start pilot | ждёт web cutover (отдельная приёмка), eligibility, инструкции с полем `feedback` и GO на issuance (J) |
...
| 11 | Cutover: реестр статусов пакетов, `v0` не выдаётся runtime'ом, pc-08 в живом glossary `Domain.hs` перефразирован, критерий eligibility по ledger — **отдельная приёмка после сборки v0.1** (build ≠ web cutover), см. [pilot-v0.1-cutover-contract](../pilot-v0.1-cutover-contract.md) | — |
| 12 | Pilot | — |
...
**Статус: naturalness-authoring CLOSED · 13/13 donor candidates accepted · v0.1 BUILT and sealed 2026-09-08 (contract A–I) · not issued · web cutover not done · human naturalness evidence: none · next gates: web cutover (отдельная приёмка), затем issuance J.** Сборка — отдельная работа с отдельным «го» по [pilot-v0.1-cutover-contract](../pilot-v0.1-cutover-contract.md): пять развилок закрыты 2026-09-08 (retirement в manifest, fresh ids, design intent facilitator-only, dogfood sidecar, exact accepted→built equality как gate), контракт A–J, build ≠ web cutover; три пункта ещё открыты и названы там (правило нумерации ids, `accepted_via` для replacement без parent, `origin` для carried_over). «Собрать как очевидно» не разрешено.
