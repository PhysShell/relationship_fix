...
Статус: research note + принятое направление (2026-09-07). Этот документ ничего не меняет в корпусе, онтологии и коде: он фиксирует, почему часть реплик звучит «пластмассово», что из готового было проверено и отвергнуто, и как устроен gate, который вместо этого делается.
...
- **Protocol §6** требует смесь источников, чтобы E0-циркулярность не просочилась в E1. Rewrite-pass LLM-ом по всему корпусу молча сделал бы каждый item частично LLM-авторским. При этом в `rf.pilot-item.v1` **нет поля provenance** (handwritten / adapted / synthetic / llm_minimal_edit): сегодня нельзя сказать, какие из 40 items какого происхождения, а после любого edit-pass это станет невосстановимо. Дыру закрывать до pass'а, не после.
...
- **Побочная находка, вне naturalness.** `behavior-v0.2-candidate.json` использует как примеры B.AVOIDANCE_TOPIC_SHIFT тексты, дословно совпадающие с target-репликами pilot-items pc-08 («Слушай, а ты видела, что Лена выложила?»), pc-09 («Давай сегодня в восемь спокойно сядем и обсудим, сейчас я на созвоне.») и pc-16 («Ясно.»). v0.1 (к которому привязан pilot) и `Domain.hs` annotation-web этого не делают — там парафразы. Commit `07b5106` фиксировал правило «ontology example texts not reused»; v0.2-candidate нарушает его в обратную сторону. Если pilot-items когда-либо предъявляются под v0.2-wording, разметчик увидит ответ в глоссарии. Решение — вне этого документа; здесь только зафиксировано.
...
5. **Provenance** — принятый edit пишется как новая версия item с `authoring: llm_minimal_edit | human_edit` и ссылкой на оригинал; оригинал не переписывается задним числом (тот же принцип, что «historical artifacts stay historical» в `data/pilot/v0/form/README.md`).
...
Кандидат 1: A «Ну да, опять не предупредил. Знаю.» / B «Я зря наорала, да. Но ты можешь просто писать, когда задерживаешься? Вот прям каждый раз.»
Кандидат 2: A «Ладно, мой косяк, что не написал.» / B «Зря орала, извини. Но если задерживаешься — напиши, ну правда. Каждый раз.»
...
Checker: извинение есть («зря наорала» / «зря орала, извини»); требование есть («можешь просто писать» / «напиши»); «но» без контратаки; род сохранён («не предупредил», «не написал» — м.; «наорала», «орала» — ж.); A теперь читается как реплика в треде, а не как признание-заставка.
...
Что из этого следует для ближайших шагов (решения, а не действия этого документа):

1. **Pilot-v0 остаётся как выдан**, если только пакет не перегенерируется целиком до открытия первым eligible разметчиком; частичная правка in place запрещена. Если 13 flagged items правятся до pilot — это `annotation-pilot-v0.1` с новым hash items, новым manifest и записью, какие item-версии заменили какие.
2. **Отчёт pilot** получает предзарегистрированную таблицу item × `unnatural_example` × disagreement. Это единственный способ отделить bad stimulus от bad definition post hoc.
3. **`rf.pilot-item.v2`** добавляет `authoring` (handwritten | adapted | synthetic | llm_minimal_edit | human_edit) и опционально `naturalness_review {flags, severity, decision, ab_votes}`. Без этого поля никакой edit-pass не запускается.
4. **v0.2-candidate**: добавить positives в живом регистре рядом с существующими (не вместо): «Да, блин, мой косяк» для принятия ответственности, «я бы тоже взбесилась» для validation, «ну напиши ты просто, сколько можно» для pressure — и убрать дословные target-тексты pilot-items из примеров (§3, побочная находка).
5. **Инструмент** (LLM-критик с few-shot из принятых и отклонённых пар) — только с полным exercise 50–100+ единиц; до этого ручной проход дешевле и надёжнее.
