...
MessageNode           L1.5  сообщение как структура: actor, order, timestamp, topic?, reply_to?
MessageEdge           L1.5  отношение между СООБЩЕНИЯМИ, считается до разметки
BehaviorObservation   L2    label + actor + message_id + evidence (непустой!)
RelationEdge          L2.5  MessageEdge + наблюдения на его концах
RelationalEvent       L3    событие, собранное ИЗ ОТНОШЕНИЙ
StateHypothesis       L4    гипотеза с собственным ledger'ом
```

### L1.5 — почему `topic` уехал с наблюдения на сообщение

`topic` — свойство **того, что было сказано**, а не поведения, которое кто-то на этом закодировал. Пока он жил на `BehaviorObservation`, неразмеченная реплика не имела темы вовсе и выпадала из топологии. Теперь тема на `MessageNode`, и `topic_continuity` / `topic_discontinuity` считаются между сообщениями независимо от того, нашла ли онтология там что-нибудь интересное.

`reply_to` поддержан, но необязателен: мессенджеры дают его не всегда. Когда он есть — это `EXPLICIT_REPLY_TO` и более сильное основание, чем `NEXT_BY_OTHER_ACTOR`.
...
```
adjacent_cross_actor_turn · explicit_reply
topic_continuity · topic_discontinuity
negative_response · softening_response
```

Каждое обосновано `MessageRelation`: у `RelationEdge` есть `via_message_relation`, и без него ребро не существует — тест требует непустого значения.

Три имени выброшены **потому, что они утверждали больше, чем видно**:

| Выброшено | Чем заменено | Почему |
|---|---|---|
| `responds_to` (из смежности) | `adjacent_cross_actor_turn` + отдельно `explicit_reply` | «следующая реплика другого» может быть ответом, новым сообщением, параллельной темой или реакцией на старое. `RESPONDS_TO` стал **эмпирическим** отношением для primitives-пилота |
| `non_uptake` | `topic_discontinuity` | «другая тема» ≠ «присутствует, но не вовлекается»: ответ может принять тему и расширить её |
| `escalates` | `negative_response` | эскалация предполагает конфликт во времени; смежная негативная пара её не показывает |

Также выброшены `follows` (выводится из порядка), `repeats`, `contradicts`, `repairs` (выводы уровня события) и `interrupts` (в асинхронном чате прерывание не наблюдаемо).

### Coding frame

`_outcome()` — единственная лестница разрешения отклика, через которую проходит каждый паттерн. Её третья ступень существует из-за одного факта: **отсутствие метки на сообщении означает либо «проверили и нет», либо «здесь не размечали»**, и различить их может только явно объявленный coding frame.

Поэтому отклик **вне frame** даёт `INSUFFICIENT_OBSERVATION`, а не `OBSERVED_ABSENCE`. Кейсы `adv-frame-01` / `adv-frame-02` в корпусе — одни и те же сообщения с разным объявленным frame и разным исходом.

Если `coded_messages` не объявлен, спайк использует прокси «закодировано = есть хоть одно наблюдение». Это **прокси, а не факт**, и настоящий pipeline обязан объявлять frame отдельно.

`topic` подаётся корпусом наравне с labels, но **на сообщении**. Без темы топиковые отношения не порождаются вовсе.
...
| Отдельный `Turn` type | слой понадобился (`MessageNode`), отдельный domain type — нет |
| Параллельный канал `opportunities` | удалён: он давал ledger'у способ старить гипотезу на эпизоде без единого события |
| Вероятности на response candidates | не нужны: достаточно основания (`explicit_reply` > `adjacency`), решает вызывающий |
