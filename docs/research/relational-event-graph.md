...
```
responds_to · continues_topic · topic_discontinuity · escalates · softens
```

Пять вместо восьми. Выброшены `follows` (тривиально выводится из порядка), `repeats`, `contradicts`, `repairs` (выводы уровня события) и `interrupts` (в речи прерывание наблюдаемо, в асинхронном чате — нет).

**`non_uptake` выброшен как примитив.** «Другая тема» ≠ «присутствует, но не вовлекается»: ответ может принять тему и расширить её. Примитив называется ровно тем, что измеряет — `topic_discontinuity`, — а non-uptake **доказывается на уровне события** дополнительным условием (тема разошлась И в отвечающем сообщении нигде нет uptake).

`responds_to` обосновывается **`MessageRelation`**, а не порядком наблюдений. Каждый `RelationEdge` несёт `via_message_relation`, и без него не существует: тест требует непустого значения на каждом ребре.

`topic` подаётся корпусом наравне с labels, но **на сообщении**. Без темы топиковые отношения не порождаются вовсе — модель молчит, а не угадывает.
