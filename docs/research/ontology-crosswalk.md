...
| `B.WITHDRAWAL` | **EPISODE** (в тексте), и даже там **не установлено** | Отсутствие вклада не локализуется. Нужно: (a) активный конфликтный эпизод, (b) базовый темп пары, (c) исключение внешних причин молчания. Единственная попытка в CMC — global над 15 мин, **ICC .52**. Exchange-уровень **не тестировал никто**. Честный статус: **text-only observability под сомнением, minimum sufficient unit не установлен**. Сильнее утверждать нельзя: IDCS-CMC этот конструкт в CMC всё-таки кодировала, а плохая reliability ≠ принципиальная неизмеримость |
