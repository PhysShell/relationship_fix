...
L2.5 InteractionRelation  направленное отношение между двумя наблюдениями
...
| Отношение | Правило |
|---|---|
| `responds_to` | следующее наблюдение **другого** актора; больше ничего откликом не считается |
| `continues_topic` | `responds_to` + та же тема |
| `non_uptake` | `responds_to` + **другая** тема (присутствует, но не подхватывает) |
...
В трейсе: `pursue_withdraw` имеет `observation_coverage = 0.50` при 5 незаполненных слотах и постоянном `unobserved_slots: ["withdrawer_internal_disengagement"]`. Три silence-only кейса дали `status: absent` и **пустой** `counterevidence` — тест `test_silence_only_cases_never_confirm_pursue_withdraw` проверяет это end-to-end.
...
`repair_softening` в трейсе теперь имеет `observation_coverage = 0.4167` — почти половина softening-ходов **не получила наблюдаемого отклика**, и это видно в числах вместо того, чтобы молча стать поддержкой. Регрессия: `test_softening_nobody_answered_is_absent_not_supporting`.
...
Политика по статусам объявлена явно константой `ABSENT_IS_AN_OPPORTUNITY = True`, а не выведена случайно:

| Статус | Старит? | Почему |
|---|---|---|
| `ABSENT` | **да** | смотрели в наблюдаемом окне — шанс был и не реализовался |
| `INSUFFICIENT_OBSERVATION` | **нет** | смотреть не могли |
| `NOT_APPLICABLE` | **нет** | слот неприменим |

Эффект виден в числах: транзиций стало **13 на 32 эпизода** против прежних 21 на 27. Тест `test_decay_needs_eligible_opportunities_not_elapsed_episodes` прогоняет шесть эпизодов без возможности — статус не меняется — и затем три реальные возможности без поддержки — статус падает.
...
|---|---|---|
| A1 | Существует нужный продукту конструкт, не выражающийся ни на одном из пяти уровней | назвать его и показать, куда он не помещается |
...
| A3 | Ledger теряет evidence, counterevidence или ненаблюдаемые слоты при любом порядке входа | property-тесты на перестановках эпизодов |
| A4 | Различение `ABSENT` / `INSUFFICIENT` / `NOT_APPLICABLE` не даёт ни одного различного решения ни на одном реальном материале | если три статуса всегда ведут себя одинаково, они лишние |
