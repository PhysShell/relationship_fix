...
| Отношение | Правило |
|---|---|
| `responds_to` | обосновано `MessageRelation`, а не порядком labels |
| `continues_topic` | `responds_to` + та же **записанная** тема |
| `topic_discontinuity` | `responds_to` + **другая** записанная тема |
| `escalates` | `responds_to` + негатив в ответ на негатив |
| `softens` | `responds_to` + softening в ответ на негатив |
...
`repair_softening` в трейсе: 5 support, 1 counter, **2 observed_absence, 10 right-censored**, `observation_coverage = 0.4444`. То есть в десяти случаях из восемнадцати запись просто обрывалась — и это теперь **не** смешано с двумя случаями, где партнёр реально ответил, но откликом это не было.
...
|---|---|---|
| `OBSERVED_ABSENCE` | **да** | окно было открыто — шанс был и не реализовался |
| `RIGHT_CENSORED` | **нет** | запись кончилась раньше исхода |
| `INSUFFICIENT_OBSERVATION` | **нет** | смотреть не могли |
| `NOT_APPLICABLE` | **нет** | слот неприменим |

### Opportunities теперь pattern-specific

Прошлая версия делала так:
...
Один-единственный негативный ход автоматически считался шансом проявиться **и** для `attack_attack`, **и** для `pursue_withdraw` — хотя второму требуется как минимум повторное преследование. Гипотеза старела на эпизодах, где её структурных предпосылок вообще не было.

Теперь каждая возможность выводится из trigger condition своего паттерна, и **каждая требует наблюдаемого окна отклика**:

| Паттерн | Условие возможности |
|---|---|
| `attack_attack` | негативный ход + наблюдаемое окно отклика |
| `pursue_withdraw` | **≥2** преследующих хода одного актора + партнёр присутствует + наблюдаемое окно |
| `repair_softening` | softening-ход + наблюдаемый последующий ход партнёра |
| `constructive_alignment` | aligning-ход + наблюдаемое окно отклика |

Триггер в самом конце лога **не является** упущенным шансом: он right-censored (`test_trigger_at_the_end_of_the_log_is_not_an_opportunity`).

Эффект виден в числах: транзиций стало **8 на 37 эпизодов** против 21 на 27 в первой версии и 13 на 32 во второй. Тест `test_decay_needs_eligible_opportunities_not_elapsed_episodes` прогоняет шесть эпизодов без возможности — статус не меняется — и затем три реальные возможности без поддержки — статус падает.
...
12. **«Opportunity = в эпизоде был негатив».** Слишком широко: один негативный ход открывал окно сразу двум паттернам, включая тот, которому нужно повторное преследование. См. §5.
...
## 10. Что стало следующим вопросом

Спайк сдвинул приоритет. Проверять надо **не** named relation patterns, а этаж под ними:

```
можем ли мы надёжно размечать
InteractionRelation / episode boundaries
ДО того, как валидировать pursue_withdraw и остальную таксономию?
```

Порядок именно такой. Если два человека не воспроизводят примитивы —

```
responds_to · continues_topic · non_uptake · escalates · softens
```

— то строить над ними `pursue_withdraw` бессмысленно: higher-order pattern будет наследовать ненадёжность основания и добавлять свою. Если воспроизводят, то паттерн становится **композицией наблюдаемых вещей**, а не ещё одним магическим label, и его разметка перестаёт быть отдельным актом веры.

Это существенно более фундаментальная находка, чем любой новый label: мы начинали с «давайте заведём DyadicState», а пришли к тому, что недостающим был этаж **под** событиями, а не ещё одна таксономия над ними.
