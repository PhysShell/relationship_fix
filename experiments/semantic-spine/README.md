...
eval/protocol.py     фазы, surface/treatment digest, transition_problems()
eval/protocol.json   версия и фаза, точка заморозки, дайджесты, запись v1, план v2
...
tests/               108 тестов: мутационный набор (19) + машина перехода v1->v2
```
...
### v2 нельзя открыть одним коммитом

Концептуального соблюдения порядка мало: можно поднять версию, починить
рендеринг и раскрыть задачи разом, и человек скажет «порядок соблюдён», а Git
увидит одну кашу.

Научно существенная граница — **B/C**, а не A/B. Разделять «написали код» и
«записали его хеш» на два коммита красиво, но независимости не добавляет, пока
и то и другое скрыто от curator'а. Поэтому A и B могут быть одним коммитом:

| ступень | ставит | содержит | не содержит |
|---|---|---|---|
| A+B | `protocol_version: 2`, `phase: v2_implementation_frozen`, `preregistered_treatment_digest` | починка `materialized_in` и преригистрация | held-out задач |
| C | `phase: v2_tasks_revealed`, `preregistered_at_commit` | манифесты held-out задач | — |

В фазах **после раскрытия** — и в `v2_tasks_revealed`, и в
`v2_experiment_complete` — обязаны выполняться все пять:

    protocol_version == 2
    preregistered_at_commit — строгий предок HEAD
    treatment_digest_at(preregistered_at_commit) == preregistered_digest
    treatment_digest()                           == preregistered_digest
    held-out задач в преригистрационном коммите ещё нет

Проверки **монотонны**: завершение эксперимента не снимает заморозку. Иначе
`phase: v2_experiment_complete` уничтожал бы provenance ровно после того, как
на прогоны потратили деньги.

Четвёртая строка без третьей ничего не доказывает. Именно на этом держался
самый интересный обход: указать на любой старый коммит, где задач ещё не было,
но treatment был **другим**. Коммит существует, не равен HEAD, задач в нём
восемь, текущий дайджест совпадает с записанным — проходило всё. Поэтому
дайджест теперь считается **из дерева названного коммита** (`treatment_digest_at`),
а не из рабочего дерева, и обе раскладки байт строит одна функция: разойдись
они, сравнение перестало бы что-либо значить, оставшись таким же убедительным.

Закрытые обходы, каждый со своим тестом в `TransitionBarrier`:

    наивный монолит (без преригистрационного коммита)
    хитрый монолит (старый коммит с другим treatment)
    побег фазой в v2_experiment_complete
    фаза v2 без поднятия protocol_version
    поздний «безобидный рефакторинг» после раскрытия

И зеркало: легальный путь обязан **пройти**. Барьер, который блокирует всё,
ошибается просто в другую сторону.

В v1 байт-равенство снимков фиксировало **наблюдавшееся** поведение; в v2
измеряется поведение агента, и семантика реализации становится частью
воздействия, а не измерения. Поэтому treatment хешируется отдельно
(`TREATMENT_FILES` в `eval/protocol.py`) и преригистрируется.
