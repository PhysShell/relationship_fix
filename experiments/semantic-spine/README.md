...
tests/               82 теста, включая мутационный набор из 19 мутаций
```
...
`88f6dfb` — development harness v0. Начиная с этого коммита **не меняются**:
граф, `SPINE_RELATION_ORDER`, восемь задач, их oracle-файлы и якоря, рендеринг
узлов. Любая правка здесь после того, как числа увидены, — это подгонка под
знакомый набор.

Разморозка — только вместе с held-out задачами, которых harness при
проектировании не видел. Лучший материал: исторические изменения самого
relationship_fix (коммит до фикса → описание PR как промпт → приехавший
патч/тесты как скрытый acceptance), либо задачи, написанные отдельной сессией,
которой не показывают ни relation order, ни реализацию селектора.
