...
tests/               103 теста: мутационный набор (19) + машина перехода v1->v2
```
...
увидит одну кашу. Поэтому переход — трёхступенчатое состояние:

| ступень | ставит | содержит | не содержит |
|---|---|---|---|
| A | `protocol_version: 2`, `phase: v2_implementation_frozen` | починка `materialized_in` | новых задач |
| B | `preregistered_treatment_digest` | преригистрация кода воздействия | held-out задач |
| C | `phase: v2_tasks_revealed` | манифесты held-out задач | — |

Критическое свойство, проверяемое в C:

    treatment_digest() == preregistered_treatment_digest

После первой held-out задачи ни selector, ни рендеринг, ни веса связей, ни
packing policy, ни token accounting не могут измениться без новой версии
протокола — включая «безобидный рефакторинг», который незаметно двигает
условие D. В v1 байт-равенство снимков фиксировало **наблюдавшееся** поведение;
в v2 измеряется поведение агента, и семантика реализации становится частью
воздействия, а не измерения. Поэтому treatment хешируется отдельно
(`TREATMENT_FILES` в `eval/protocol.py`) и преригистрируется.

Дайджест, записанный тем же коммитом, что раскрывает задачи, совпадает по
построению и ничего не доказывает. Поэтому преригистрация обязана **называть
отдельный коммит-предок**, и в нём held-out задач ещё не должно быть — это
проверяется по git-истории.

До этого монолит ловила ровно одна проверка: голое `version() == 1`. Это был не
барьер, а табличка «удали меня при открытии v2».

Всё вместе: `eval/protocol.json` → `v2_plan`, `transition_problems()` в
`eval/protocol.py`, и три класса тестов — `BlindBarrier`, `StagedTransition`,
`StagedTransitionIsSatisfiable`. Последний — зеркало мутационного набора:
он строит настоящий git-репозиторий, проходит A → B → C и требует, чтобы
легальный путь **прошёл**, а поздний рефакторинг и монолитный коммит были
отвергнуты. Барьер, который блокирует всё, ошибается просто в другую сторону.
